...
contract ICO is Owned, Controller {  
  //ICO es el controlador y propietario, tiene el método proxypayment y permite que des del token "controlado" se pueda avisar a ICO para que haga operación.
    ...
    address public ICOAccount; //dirección de la ICO, irán todos los Ethers pagados
    ...
    function initialize ( address _token, address _icoAccount) public  { 
        require(address(token) == 0x0); //comprobamos que no se inicializa más de una vez
        token = Token(_token); //creamos Token con dirección que nos dan
        require(token.totalTokensSupply() == 0); //comprobamos datos a 0, que no sea token usado
        require(token.controller() == address(this)); //controller debe ser la address owner (ICO)
    ...
    function doBuy(address _sender, uint _value) public returns (bool) { // función de compra
        ...
        require (totalCollected + _value <= limit); // evaluamos que no superemos el límite, si nos pasamos bloqueamos
    ...
    function() public payable {  //función que es fallback, se hace por si un usuario envía dinero a la address de la ICO
