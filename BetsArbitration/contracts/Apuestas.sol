...
contract Apuestas {
    ...
    address payable public jugador1;
    address payable public jugador2;
    ...
    event logApuestaGanadora(string _value);  //para escribir logs

    modifier soloEstado (Estado estadoEsperado) {  //modifier lleva restricciones para functiones que lo pidan.
            require(estadoEsperado == estadoActual, "ERROR: estadoEsperado is diferent than estadoActual");
                //Comprueba que el estado actual corresponda con esperado
            _;
    }
    ...
    function aceptarApuesta() public soloEstado(Estado.apuestaHecha) payable { //recibe from y value (payable), que irá a Apuestas address.
        require(msg.value == apostado, "ERROR: value is diferent than apostado");
    ...
    function resolverApuesta() public soloEstado(Estado.apuestaAceptada)  { //recibe from y value (payable), que irá a Apuestas address.
        ...
        if (flip == 0){
            emit logApuestaGanadora("Gana Jugador1");
            jugador1.transfer(address(this).balance);
        } else{
            emit logApuestaGanadora("Gana Jugador2");
